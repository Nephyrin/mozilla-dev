                                   ...
                                   nsIRequest *aLoadingChannel)
{
  ...
  // If we're a plugin type, check to see if we should show disabled or
  // click-to-play fallback instead
  ...
  switch (mType) {
    ...
    case eType_FakePlugin:
    {
      ...
      if (loadInfo) {
        loadInfo->SetOwner(thisContent->NodePrincipal());
